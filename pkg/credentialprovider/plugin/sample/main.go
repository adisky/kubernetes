...
func main() {
	...
	rootCmd := &cobra.Command{
		Use:   "docker-auth-provider",
		Short: "docker registry auth plugin",
	}
...
func getCredentials() error {
	klog.V(2).Infof("@@adisky get-credentials")
	...
	p := credentialprovider.NewDockerConfigProvider()
	dockercfg := p.Provide(authRequest.Image)
